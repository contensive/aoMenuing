...
function lmHover(event,hoverClass) {
	...
	if (lmLastItemHovered != null)
		lmClearItemAndChildList(lmLastItemHovered,hoverClass);
...
// -----
//
function lmClearItemAndChildList(listItem,hoverClass) {
	if (listItem) {
		removeClassNameAfter(listItem, hoverClass);
		if (listItem.childList != null) {
			lmCloseChildList(listItem.childList,hoverClass);
...
function lmListHover(event,StyleSheetPrefix,hoverClass) {
	...
	if (parentList.title != "") {
		if (parentList.activeItem == null) {
			...
			lmCloseChildList(parentList,hoverClass);
		}
	}
}
//
...
// -----
//
function lmCloseChildList(list,hoverClass) {
	if (!list)
		return;
	if (list.activeItem == null)
		return;
	if (list.activeItem.childList) {
		lmCloseChildList(list.activeItem.childList,hoverClass);
	...
	removeClassNameAfter(list.activeItem, hoverClass);
...
//
function lmPageClick(event,hoverClass) {
	//alert("lmPageClick");
	var el;
	...
	if (el == lmLastItemHovered)
		return true;
	if (getContainerWith(el, "DIV", hoverClass) == null) {
		lmClearItemAndChildList(lmLastItemHovered,hoverClass);
		lmLastItemHovered = null;
	}
}
