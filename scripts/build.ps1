...
# Display build info
$version = Get-ContensiveVersion -DeploymentRoot 'C:\Deployments\aoMenuing'
Write-Host ""
Write-Host "========================================"
Write-Host "  aoMenuing Build"
...
Invoke-ContensiveBuild `
    -CollectionName    'aoMenuing' `
    -CollectionPath    "$projectRoot\collections\aoMenuing" `
    -SolutionPath      "$projectRoot\server\aoMenuing.sln" `
    -BinPath           "$projectRoot\server\aoMenuing\bin\$Configuration\netstandard2.0" `
    -Configuration     $Configuration `
    -DeploymentRoot    'C:\Deployments\aoMenuing' `
    -CleanFolders      @(
                           "$projectRoot\server\aoMenuing\bin"
